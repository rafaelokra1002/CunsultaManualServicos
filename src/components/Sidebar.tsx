...
function ResetECMIcon({ active }: { active?: boolean }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={active ? "#ff6a1a" : "currentColor"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
...
const navItems = [
  ...
  { href: "/reset-ecm", label: "Reset / Init ECM", Icon: ResetECMIcon },
];
    ...
    <>
      ...
      <div className="fixed inset-x-0 top-0 z-50 flex h-16 items-center justify-between border-b border-[#33373f] bg-[#111317]/95 px-4 backdrop-blur md:hidden">
        <button
          ...
          className="flex h-10 w-10 items-center justify-center rounded-xl border border-[#33373f] bg-[#1e2127] text-white transition-colors hover:border-[#ff6a1a]/30"
          aria-label="Abrir menu"
        >
          <MenuIcon />
        </button>
        <Link href="/dashboard" className="flex items-center text-lg font-bold text-white">
          <Logo size="sm" /> <span className="ml-2">Oficina<span className="text-[#ff6a1a]">Digital</span></span>
        </Link>
      ...
      <aside className={`fixed left-0 top-0 z-50 flex h-screen w-[84vw] max-w-[320px] flex-col border-r border-[#33373f] bg-[#111317] transition-transform duration-300 md:z-40 md:w-64 md:max-w-none ${open ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
        ...
        <div className="flex h-16 items-center justify-between border-b border-[#33373f] px-6">
          ...
          <button
            type="button"
            onClick={() => setOpen(false)}
            className="flex h-8 w-8 items-center justify-center rounded-lg text-[#9aa1ac] transition-colors hover:bg-[#1e2127] hover:text-white md:hidden"
        ...
        <nav className="flex-1 space-y-1 px-3 py-5">
          <p className="mb-3 px-3 text-[10px] font-bold uppercase tracking-widest text-[#6c727c]">Menu</p>
          {navItems.map((item) => {
            ...
            return (
              <Link
                ...
                className={`group relative flex items-center gap-3 rounded-xl px-3 py-3 text-sm font-medium transition-all duration-200 ${
                  isActive
                    ? "bg-gradient-to-r from-[#ff6a1a]/15 to-[#ff6a1a]/5 text-[#ff6a1a] shadow-sm shadow-[#ff6a1a]/10"
                    : "text-[#9aa1ac] hover:bg-[#1e2127] hover:text-white"
                }`}
              >
                {isActive && (
                  <span className="absolute left-0 top-1/2 h-6 w-1 -translate-y-1/2 rounded-r-full bg-[#ff6a1a]" />
                )}
                <div className={`flex h-8 w-8 items-center justify-center rounded-lg transition-colors ${
                  isActive ? "bg-[#ff6a1a]/20" : "bg-[#1e2127] group-hover:bg-[#33373f]"
                }`}>
        ...
        {/* Footer */}
        <div className="space-y-3 border-t border-[#33373f] p-4">
          <InstallButton />
          ...
          {/* User card */}
          <div className="flex items-center gap-3 rounded-xl bg-[#111317] p-3">
            <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-gradient-to-br from-[#ff6a1a] to-[#ff8c3f] text-xs font-bold text-white">
              {initials}
            </div>
            <div className="min-w-0 flex-1">
              <p className="truncate text-sm font-semibold text-white">{session?.user?.name}</p>
              <p className="truncate text-[11px] text-[#9aa1ac]">{session?.user?.email}</p>
            </div>
          </div>
          ...
          <button
            ...
            className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium text-[#9aa1ac] transition-all hover:bg-red-500/10 hover:text-red-400"
          >
            <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#1e2127]">
              <LogoutIcon />
            </div>
            Sair
          </button>
