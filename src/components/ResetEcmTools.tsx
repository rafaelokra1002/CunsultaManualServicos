...
export function FerramentaInicializacao() {
    ...
    <div className="space-y-4">
      ...
      {!allDone ? (
        <div className={`rounded-2xl border ${c.border} bg-[#0f0f18] p-5`}>
          ...
          {step.timer && (
            <CountdownTimer seconds={step.timer} onDone={() => setTimerDone(true)} />
          )}
...
export function FerramentaResetCombustivel() {
  const [currentStep, setCurrentStep] = useState(0);
  const [completed, setCompleted] = useState<number[]>([]);
  const [timerDone, setTimerDone] = useState(false);
...
function PulseDiagram({ pulses, barClass }: { pulses: number; barClass: string }) {
  ...
  for (let i = 0; i < pulses; i++) {
    segments.push({ on: true, w: 14 });
    if (i < pulses - 1) segments.push({ on: false, w: 10 });
  }
  segments.push({ on: false, w: 36 });
