  ...
  return (
    <div className="flex h-[calc(100vh-5rem)] flex-col pb-20 sm:pb-0 md:h-[calc(100vh-4rem)]">
      ...
      <div className="mb-4 flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-gradient-to-br from-[#ff6a1a] to-[#ff8c3f] text-lg">
          🤖
        </div>
        <div>
        ...
        <span className="ml-auto rounded-full bg-[#ff6a1a]/20 px-3 py-1 text-xs font-semibold text-[#ff6a1a]">
          GPT-4o mini
        </span>
      </div>
      ...
      {/* Chat area */}
      <div className="flex-1 overflow-y-auto rounded-2xl border border-[#33373f] bg-[#111317] p-4">
        {messages.length === 0 ? (
            ...
            <div className="grid w-full max-w-lg gap-2 sm:grid-cols-2">
              {SUGGESTIONS.map((s) => (
                <button
                  ...
                  disabled={!isPremium}
                  className="rounded-xl border border-[#33373f] bg-[#111317] px-3 py-2.5 text-left text-xs text-[#9aa1ac] transition-all hover:border-[#ff6a1a]/40 hover:text-white disabled:cursor-not-allowed disabled:opacity-40"
                >
                  {s}
                </button>
              ))}
          ...
          <div className="space-y-4">
              ...
              >
                <div
                  className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-lg text-sm ${
                    msg.role === "user"
                      ? "bg-gradient-to-br from-[#ff6a1a] to-[#ff8c3f]"
                      : "bg-[#1e2127]"
                  }`}
                >
                  {msg.role === "user" ? "👤" : "🤖"}
                ...
                <div
                  className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-relaxed ${
                    msg.role === "user"
                      ? "bg-[#ff6a1a]/20 text-white"
                      : "bg-[#1e2127] text-[#d0d0e8]"
                  }`}
                >
                  {msg.content.split("\n").map((line, j) => (
                    <span key={j}>
            ...
            {loading && messages[messages.length - 1]?.role === "user" && (
              <div className="flex gap-3">
                <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-[#1e2127] text-sm">
                  🤖
                </div>
                <div className="rounded-2xl bg-[#1e2127] px-4 py-3">
                  <div className="flex gap-1">
                    <span className="h-2 w-2 animate-bounce rounded-full bg-[#ff6a1a]" style={{ animationDelay: "0ms" }} />
                    <span className="h-2 w-2 animate-bounce rounded-full bg-[#ff6a1a]" style={{ animationDelay: "150ms" }} />
                    <span className="h-2 w-2 animate-bounce rounded-full bg-[#ff6a1a]" style={{ animationDelay: "300ms" }} />
                  </div>
                </div>
              </div>
            )}
      ...
      <form onSubmit={handleSubmit} className="mt-3 flex gap-2">
        <textarea
          ...
          rows={1}
          className="flex-1 resize-none rounded-xl border border-[#33373f] bg-[#111317] px-4 py-3 text-sm text-white placeholder-[#9aa1ac] outline-none focus:border-[#ff6a1a] transition-colors disabled:cursor-not-allowed disabled:opacity-50"
          style={{ minHeight: "48px", maxHeight: "120px" }}
        />
        <button
          type="submit"
          disabled={!isPremium || loading || !input.trim()}
          className="rounded-xl bg-[#ff6a1a] px-4 py-3 text-white transition-all hover:bg-[#5a4bd6] disabled:opacity-40 disabled:cursor-not-allowed"
        >
      ...
      <p className="mt-2 text-center text-[10px] text-[#6c727c]">
