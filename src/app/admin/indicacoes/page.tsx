  ...
  return (
    <div>
      ...
      <div className="mb-8 card-glass rounded-2xl p-6">
        ...
        <form onSubmit={createLink} className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <div className="flex-1">
            <label className="mb-1 block text-sm text-[#8888a4]">
              Nome / Rótulo
            </label>
            <input
              type="text"
          ...
          <button
            ...
            className="rounded-xl bg-[#6c5ce7] px-6 py-2.5 text-sm font-semibold text-white transition-all hover:bg-[#5a4bd6] disabled:opacity-50 whitespace-nowrap"
          >
      ...
      <div className="card-glass overflow-hidden rounded-2xl">
        {loading ? (
          <div className="p-8 text-center text-[#8888a4]">Carregando links...</div>
        ) : links.length === 0 ? (
          ...
          <>
            ...
            <div className="space-y-4 p-4 md:hidden">
              {links.map((link) => (
                <div key={link.id} className="rounded-2xl border border-[#2a2a3e] bg-[#12121a] p-4">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <p className="font-semibold text-white">{link.label}</p>
                      <p className="mt-1 font-mono text-xs text-[#6c5ce7]">{link.code}</p>
                    </div>
                    <span className="rounded-md bg-[#1a1a2e] px-2.5 py-1 text-xs text-[#8888a4]">
                      {link.usedCount} uso{link.usedCount !== 1 ? "s" : ""}
                    </span>
                  </div>

                  <p className="mt-2 break-all text-xs text-[#8888a4]">
            ...
            <div className="hidden overflow-x-auto md:block">
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-[#2a2a3e]">
                  {links.map((link) => (
                    <tr key={link.id} className="transition-colors hover:bg-[#1a1a2e]">
                      <td className="px-6 py-4 font-medium text-white">{link.label}</td>
                      <td className="px-6 py-4 font-mono text-[#6c5ce7]">{link.code}</td>
                      <td className="px-6 py-4 max-w-xs">
                        <span className="truncate block text-xs text-[#8888a4]">
